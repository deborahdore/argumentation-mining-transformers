...
class SequenceTaggingDataset(BaseDataset):
	...
	def _load_dataset(self,
		...
		if self.label2id is None:
			...
			# single label)
			self.label2id = {lbl: idx for idx, lbl in enumerate(  # Get label and index
		...
		self.dataset = [{"tokens": sentence['tokens'], "labels": [self.label2id[lbl] for lbl in sentence['labels']]}
						for
						sentence in sentences]

	def _tokenize_and_align_labels(self, sentence: Dict[str, Union[List[str], List[int]]]) -> BatchEncoding:
		"""
		...
		"""
		tokenized_sentence = self.tokenizer(sentence["tokens"],
		...
		for wid, attmsk in zip(word_ids, tokenized_sentence['attention_mask']):
			if wid is None:
				if attmsk == 1:
					...
					# the 'PAD' label
					extension_label = self.label2id['X'] if self.use_extension_label else self.label2id['PAD']
	...
	def __getitem__(self, idx) -> BatchEncoding:
		if isinstance(idx, slice):
			...
			tokenized_sentences = [self._tokenize_and_align_labels(sentence) for sentence in self.dataset[idx]]
			...
			return BatchEncoding({key: [ts.get(key, []) for ts in tokenized_sentences] for key in
								  set(chain(*[ts.keys() for ts in tokenized_sentences]))})
		else:
...
class SequenceTaggingDataModule(BaseDataModule):
	...
	tokens).
	"""
	LABELS = ["PAD", "O", "B-Claim", "I-Claim", "B-Premise", "I-Premise", "B-Marker", "I-Marker", "B-Treatment",
			  "I-Treatment", "B-Disease", "I-Disease", "B-Diagnostics", "I-Diagnostics"]

	@property
	def collate_fn(self) -> Callable:
		return DataCollatorForTokenClassification(self.tokenizer, label_pad_token_id=self.label2id['PAD'])

	def decode_predictions(self,
						   input_ids: Union[List[int], List[List[int]]],
						   predictions: Union[List[int], List[List[int]]],
						   labels: Union[List[int], List[List[int]], None] = None) -> Union[
		...
		if isinstance(input_ids[0], int):
			special_tokens = self.tokenizer.get_special_tokens_mask(input_ids, already_has_special_tokens=True)
			if labels is None:
				return [(self.tokenizer.convert_ids_to_tokens(token), self.id2label[prediction]) for
						token, prediction, mask in zip(input_ids, predictions, special_tokens) if mask == 0]
			else:
				return [(self.tokenizer.convert_ids_to_tokens(token), self.id2label[prediction], self.id2label[label])
						for token, prediction, label, mask in zip(input_ids, predictions, labels, special_tokens) if
						mask == 0]
		else:
			outputs = []
			if labels is None:
				for stokens, spredictions in zip(input_ids, predictions):
					special_tokens = self.tokenizer.get_special_tokens_mask(stokens, already_has_special_tokens=True)
					outputs.append([(self.tokenizer.convert_ids_to_tokens(token), self.id2label[prediction]) for
									token, prediction, mask in zip(stokens, spredictions, special_tokens) if mask ==
									0])
			else:
				for stokens, spredictions, slabels in zip(input_ids, predictions, labels):
					special_tokens = self.tokenizer.get_special_tokens_mask(stokens, already_has_special_tokens=True)
					outputs.append([
						(self.tokenizer.convert_ids_to_tokens(token), self.id2label[prediction], self.id2label[label])
						for token, prediction, label, mask in zip(stokens, spredictions, slabels, special_tokens) if
						mask == 0])
			return outputs
	...
	def _load_dataset_split(self, path_to_dataset: str) -> SequenceTaggingDataset:
		...
		"""
		return SequenceTaggingDataset(tokenizer=self.tokenizer,
									  path_to_dataset=path_to_dataset,
