...
def train_model(data_module: pl.LightningDataModule, model: pl.LightningModule, config: argparse.Namespace):
	"""
	Trains a model.
...
if __name__ == "__main__":
	...
	parser.add_argument("--model", required=True, help="Either the name of one of the available models: "
													   ...
													   "The HF model can be either a model available at the HF Hub, "
                                                       "or "
													   "a model path. To load a checkpoint reached using this same "
                                                       "trainer "
	...
	parser.add_argument("--cache-dir", default="./cache", help="Directory for Hugging Face downloaded models.")
	...
	parser.add_argument("--training-labels",
						default=None,
						nargs="*",
						help="The list of labels (separated by spaces) to keep in the dataset. "
	...
	parser.add_argument("--batch-size", default=8, type=int, help="Batch size (per GPU/CPU) for training.")
	...
	parser.add_argument("--max-seq-length", type=int, help="The maximum total input sequence length after "
														   ...
														   "If left empty it will truncate to the model's max size and "
                                                           "pad to "
	...
	parser.add_argument("--learning-rate", default=2e-5, type=float, help="The initial learning rate for AdamW.")
	...
	parser.add_argument("--crf-loss", action="store_true", help="Only useful for Sequence Tagging trainings. "
																"If true the loss function uses Conditional Random "
                                                                "Fields.")
	...
	if (config.model not in MODELS and not Path(config.model).is_file() and len(list(list_models(search=config.model)))
            == 0):
