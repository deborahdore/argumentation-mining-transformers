	...
	for split in DATA_SPLITS:
		...
		if split_file:
			if split == 'dev' and 'validation' in splits_files:
				logger.warning(f"Ignoring the file {split_file[0]} since there is {splits_files['validation']}")
				...
				logger.warning(f"There's more than 1 file for {split} split. Using {split_file[0]}.")
...
def train_model(data_module: pl.LightningDataModule, model: pl.LightningModule, config: argparse.Namespace) -> Tuple[
	pl.Trainer, ModelCheckpoint]:
	"""
	...
	model_logger = TensorBoardLogger(save_dir=config.output_dir / config.logging_dir,
									 name=model_name,
	...
	model_checkpoints = ModelCheckpoint(dirpath=checkpoint_path,
	...
	if config.early_stopping:
		early_stopping = EarlyStopping(monitor='val_loss', min_delta=1e-6, patience=config.early_stopping)
	...
	trainer = pl.Trainer(accelerator=config.accelerator,
						 ...
						 # Use only 10% of training for debug
...
def evaluate_model(data_module: pl.LightningDataModule,
	...
	if isinstance(model_or_checkpoint, pl.LightningModule):
		decoded_predictions = [decoded_prediction for batch_prediction in
							   trainer.predict(model=model_or_checkpoint, datamodule=data_module) for
							   decoded_prediction
							   in data_module.decode_predictions(**batch_prediction)]
	else:
		decoded_predictions = [decoded_prediction for batch_prediction in
							   trainer.predict(ckpt_path=model_or_checkpoint, datamodule=data_module) for
							   decoded_prediction in data_module.decode_predictions(**batch_prediction)]
	...
	if config.task_type == 'rel-class':
		...
		metrics = compute_metrics(true_labels, pred_labels, relevant_labels=relevant_labels, prefix="eval")
		...
		with open(results_dir / f'{model_name}_predictions.conll', 'w') as fh:
			print("\n\n".join(["\n".join(["\t".join(token) for token in sentence]) for sentence in
							   decoded_predictions]), file=fh)
		if config.relevant_labels is not None:
			relevant_labels = config.relevant_labels
		else:
			relevant_labels = [lbl for lbl in data_module.label2id.keys() if lbl not in {'X', 'PAD'}]
		metrics = compute_metrics(true_labels, pred_labels, relevant_labels=relevant_labels, prefix="eval")
		seq_tag_metrics = compute_seq_tag_labels_metrics(true_labels,
														 pred_labels,
	...
	with open(results_dir / f'{model_name}_report.txt', 'wt') as fh:
		print(classification_report(true_labels,
									...
									zero_division=0), file=fh)

	return metrics


def evaluate_models(data_module: pl.LightningDataModule,
	...
	if not config.train:
		...
		model_logger = TensorBoardLogger(save_dir=config.output_dir / config.logging_dir,
										 name=model_name,
										 version=config.timestamp)
		model_checkpoints = ModelCheckpoint(dirpath=config.output_dir / config.checkpoint_path,
											filename=model_name + "_{epoch:02d}_{step:05d}")
		trainer = pl.Trainer(accelerator=config.accelerator,
							 devices=config.num_devices,
							 precision='16-mixed' if config.fp16 else '32-true',
	...
	if config.eval_all_checkpoints:
		...
		for checkpoint_file in sorted(Path(model_checkpoints.dirpath).glob(f'{model_name}_*.ckpt')):
			...
			if last_checkpoint_step > 0 and checkpoint_step is None:
				...
				continue
			elif (checkpoint_step is not None and checkpoint_step < last_checkpoint_step) or last_checkpoint_step == 0:
				...
				if not config.train:
					...
					checkpoint_file = TASKS[config.task_type][1].load_from_checkpoint(checkpoint_file)
				metrics = evaluate_model(data_module, checkpoint_file, config, trainer, checkpoint_name)
		...
		if last_model_checkpoint is not None:
			...
			metrics = evaluate_model(data_module, last_model_checkpoint, config, trainer, final_model_name)
...
if __name__ == "__main__":
	...
	parser.add_argument("--train-data", type=Path, help="The train dataset path. It should already be in the format "
														"for the corresponding task (`--task-type`).")
	parser.add_argument("--test-data", type=Path, help="The test dataset path. It should already be in the format "
													   "for the corresponding task (`--task-type`).")
	...
	parser.add_argument("--model", required=True, help="Either the name of one of the available models: "
													   f"{', '.join(MODELS.keys())}; or a path to a pre-trained model "
													   f"file. "
													   "If using a file, it must be a Hugging Face model path. To "
													   "load "
													   "a "
													   "checkpoint reached using this same trainer script please use "
													   "the "
													   "`--load-from-checkpoint` option.")

	# Other parameters
	parser.add_argument("--config", help="Pretrained config name or path (if not the same as `model`).")
	parser.add_argument("--tokenizer", help="Pretrained tokenizer name or path (if not the same as `model`).")
	parser.add_argument("--cache-dir", help="Directory for Hugging Face downloaded models.")
	...
	parser.add_argument("--load-from-checkpoint", help="Path to a checkpoint file to continue training.")
	...
	parser.add_argument("--accelerator", default="auto", help="What device to use as accelerator (cpu, gpu, tpu, "
															  "etc).")
	...
	parser.add_argument("--max-steps",
						...
						help="If > 0: set total number of training steps to perform. "
							 "Overrides epochs.")
	parser.add_argument("--train-batch-size", default=8, type=int, help="Batch size (per GPU/CPU) for training.")
	parser.add_argument("--eval-batch-size", default=8, type=int, help="Batch size (per GPU/CPU) for evaluation.")
	...
	parser.add_argument("--max-seq-length", type=int, help="The maximum total input sequence length after "
														   "tokenization."
														   "Sequences longer than this will be truncated, "
														   "sequences shorter will be padded. "
														   "If left empty it will truncate to the model's max size "
														   "and "
														   "pad to "
														   "the maximum size of each training step.")
	parser.add_argument("--lower-case", action="store_true", help="Should be active for lowercase transformers.")
	parser.add_argument("--learning-rate", default=5e-5, type=float, help="The initial learning rate for AdamW.")
	parser.add_argument("--weight-decay", default=0.0, type=float, help="Weight decay for parameters that accept it.")
	parser.add_argument("--adam-epsilon", default=1e-8, type=float, help="Epsilon for Adam optimizer.")
	parser.add_argument("--warmup-steps", default=0, type=int, help="Number of steps for linear warmup.")
	...
	parser.add_argument("--log-every-n-steps", default=50, type=int, help="Log every N update steps.")
	parser.add_argument("--save-every-n-steps", default=50, type=int, help="Save checkpoint every N update steps.")
	parser.add_argument("--eval-all-checkpoints", action="store_true", help="Evaluate all checkpoints for the model.")
	...
	if (
			config.output_dir.exists() and list(config.output_dir.glob('*')) and config.train and not
	...
	data_module = TASKS[config.task_type][0](data_splits=data_splits,
											 tokenizer_name_or_path=hf_tokenizer_name_or_path,
											 labels=config.labels,
											 tokenizer_config=dict(cache_dir=config.cache_dir,
																   do_lower_case=config.lower_case,
																   use_fast=True,
																   add_prefix_space=True if hf_tokenizer_name_or_path
																							== 'roberta-base' else
																   False),
	...
	if config.load_from_checkpoint is not None:
		...
		model = TASKS[config.task_type][1](model_name_or_path=hf_model_name_or_path,
										   ...
										   classes_weights=data_module.classes_weights if config.weighted_loss else
										   None)
	...
	if config.evaluation_split:
		...
		evaluate_models(data_module,
						model,
						config,
						trainer if config.train else None,
