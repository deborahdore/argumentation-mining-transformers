...
class RelationClassificationDataset(BaseDataset):
	...
	def _load_dataset(self,
		...
		with open(path_to_dataset, 'rt') as fh:
			...
			if self.training_labels:
				dataset = [sublist for sublist in dataset if sublist[0].lstrip('__label__') in self.training_labels]
	...
	def __getitem__(self, idx) -> BatchEncoding:
		...
		if isinstance(idx, slice):
			...
			tokenized_data = self.tokenizer(text=text,
											text_pair=text_pair,
...
class RelationClassificationDataModule(BaseDataModule):
	...
	def decode_predictions(self,
		...
		if isinstance(input_ids[0], int):
			# Single input
			sentence1 = self.tokenizer.decode(input_ids[:input_ids.index(self.tokenizer.sep_token_id)],
											  skip_special_tokens=True)
			sentence2 = self.tokenizer.decode(input_ids[input_ids.index(self.tokenizer.sep_token_id):],
											  skip_special_tokens=True)
			...
			if labels is None:
				...
				return [(self.id2label[label], self.id2label[prediction],
						 self.tokenizer.decode(input_id[:input_id.index(self.tokenizer.sep_token_id)],
											   skip_special_tokens=True),
						 self.tokenizer.decode(input_id[input_id.index(self.tokenizer.sep_token_id):],
											   skip_special_tokens=True)) for label, prediction, input_id in
						zip(labels, predictions, input_ids)]

	def _load_dataset_split(self, path_to_dataset: str) -> RelationClassificationDataset:
		"""
		Check BaseDataModule for documentation.
		"""
		return RelationClassificationDataset(tokenizer=self.tokenizer,
											 path_to_dataset=path_to_dataset,
