...
if __name__ == "__main__":
	...
	arg_comp_parser = subparsers.add_parser("components",
											parents=[base_parser],
											help="Runs the Argumentative Components Detection "
												 "pipeline.")
	arg_comp_parser.add_argument("--arg-comp-model",
								 "-c",
	...
	arg_struct_parser = subparsers.add_parser("structure",
											  parents=[arg_comp_parser],
											  add_help=False,
											  help="Runs the Argumentative Structure "
												   "Classification pipeline.")
	arg_struct_parser.add_argument("--arg-struct-model",
								   "-s",
	...
	if args.pipeline in {'components', 'structure'}:
		...
		arg_comp_model = SequenceTaggingTransformerModule.load_from_checkpoint(args.arg_comp_model)

		# FIXME: We use the default tokenizer and id2label present in the config
		logger.info("Tagging argumentative components.")
		arg_comp_annotations, arg_comp_relevant = sequence_tagging(text=input_text,
		...
		annotations['argumentative_components'] = {'annotations': arg_comp_annotations, 'relevant': arg_comp_relevant}

	if args.pipeline == 'structure':
		logger.info("Loading argumentative structure model.")
		arg_struct_model = RelationClassificationTransformerModule.load_from_checkpoint(args.arg_struct_model)
		...
		rel_classes = relation_classification(text=[arg_comp_annotations[comp['ref']]['text'] for comp in
													source_components],
											  text_pair=[arg_comp_annotations[comp['ref']]['text'] for comp in
														 target_components],
		...
		for source, target, relation in zip(source_components, target_components, rel_classes):
			if args.filter_no_rel_class and relation == args.no_rel_class:
				continue
			relation_annotation = {'source': source['id'], 'target': target['id'], 'relation': relation['label']}
			if args.confidence:
